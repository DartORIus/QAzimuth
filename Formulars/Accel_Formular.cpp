...
{
   // Graphic::Build_Graphic(ACCEL.X_V,     ACCEL.Status, 0);
   // Graphic::Build_Graphic(ACCEL.Y_V,     ACCEL.Status, 1);
   // Graphic::Build_Graphic(ACCEL.Z_V,     ACCEL.Status, 2);
   // Graphic::Build_Graphic(Move,          ACCEL.Status, 3);

 //   float X = ACCEL.X_V - (-2.15126707) * ACCEL.T;
 //   float Y = ACCEL.Y_V - (-1.03883850) * ACCEL.T;
 //   float Z = ACCEL.Z_V - (2.428953565) * ACCEL.T;
...
//    X = asin(X/16384.0)*180.0/PI;
//    Y = asin(Y/16384.0)*180.0/PI;
//    Z = acos(Z/16384.0)*180.0/PI;
   ...
   // emit Accel_Deg_SIGNAL(y_Grad, Status);
}
